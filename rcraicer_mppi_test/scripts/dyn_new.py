...
class Dyn():
    ...
    def test(self):
        ...
        df = pd.DataFrame(data=results, dtype=results.dtype, columns=["roll_out", "x_out", "y_out", "yaw_out", "roll", "x", "y", "yaw", "loss", "steer", "throttle"])
        ...
        for i in range(0, 10):
            ...
            if self.interactive:
                print("Index: {} loss: {} x: {} y: {} yaw: {} roll: {} x_out: {} y_out: {} yaw_out: {} roll_out: {}".format(index, sorted_df.loss[index],sorted_df.u_x[index],
                                    sorted_df.y[index], sorted_df.yaw[index], sorted_df.roll[index], sorted_df.x_out[index], sorted_df.y_out[index], sorted_df.yaw_out[index], sorted_df.roll_out[index]))

                self.log_verbose("Index: {} loss: {} x: {} y: {} yaw: {} roll: {} x_out: {} y_out: {} yaw_out: {} roll_out: {}\n".format(index, sorted_df.loss[index],sorted_df.x[index],
                                    sorted_df.y[index], sorted_df.yaw[index], sorted_df.roll[index], sorted_df.x_out[index], sorted_df.y_out[index], sorted_df.yaw_out[index], sorted_df.roll_out[index]))

        fig, axarr = plt.subplots(3, 2, figsize=(12,8))
        loss_plot = df.plot(y="loss", kind="line", ax=axarr[0][0])
        x_out_plot = df.plot(y="x_out", kind="line", ax=axarr[0][1])
        x_out_plot = df.plot(y="x", kind="line", ax=axarr[0][1])
        y_out_plot = df.plot(y="y_out", kind="line", ax=axarr[1][0])
        y_out_plot = df.plot(y="y", kind="line", ax=axarr[1][0])        

        yaw_out_plot = df.plot(y="yaw_out", kind="line", ax=axarr[1][1])
        yaw_out_plot = df.plot(y="yaw", kind="line", ax=axarr[1][1])
        ...
        throttle_out_plt = df.plot(y="x_out", kind="line", ax=axarr[2][1])
