#include "../include/rcraicer_mppi_test/sim_data_recorder.h"

using std::placeholders::_1;
using namespace std::chrono_literals;


SimDataRecorder::SimDataRecorder() : Node("sim_data_recorder_node")
{
    paramSetCallbackHandler = this->add_on_set_parameters_callback(std::bind(&SimDataRecorder::paramSetCallback, this, std::placeholders::_1));

    updateInternalParams();
 
    ssSubscription = this->create_subscription<rcraicer_msgs::msg::SimState>(
      "sim_state", 10, std::bind(&SimDataRecorder::sim_state_callback, this, std::placeholders::_1));       

    joySubscription = this->create_subscription<sensor_msgs::msg::Joy>(
        "joy", 10, std::bind(&SimDataRecorder::joy_callback, this, std::placeholders::_1));    
...
SimDataRecorder::~SimDataRecorder()
{
    closeFile();
}


rcl_interfaces::msg::SetParametersResult SimDataRecorder::paramSetCallback(const std::vector<rclcpp::Parameter>& parameters)
{
...
void SimDataRecorder::updateInternalParams()
{    
}

void SimDataRecorder::openFile()
{
    std::scoped_lock lock(fileMutex);  
...
void SimDataRecorder::closeFile()
{
    std::scoped_lock lock(fileMutex);  
...
void SimDataRecorder::sim_state_callback(const rcraicer_msgs::msg::SimState::SharedPtr msg)
{        
    if (!recordMode)
        return;
...
void SimDataRecorder::copyMessage(const rcraicer_msgs::msg::SimState::SharedPtr msg)
{
...
float SimDataRecorder::yawChange(float yaw1, float yaw2)
{        
...
float SimDataRecorder::distanceTravelled(float x, float y, float x1, float y1)
{
    float deltaX = x - x1;
    float deltaY = y - y1;
...
void SimDataRecorder::joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
{    
    rclcpp::Time ts = msg->header.stamp;    
...
int main(int argc, char * argv[])
{
    ...
    rclcpp::spin(std::make_shared<SimDataRecorder>());
    rclcpp::shutdown();
    return 0;
}
