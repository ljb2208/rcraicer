...
{   
    ...
    serialPort = new SerialPort((rclcpp::Node::SharedPtr)this, this->get_name(), "rc_chassis", portPath.as_string(), baudRate.as_int(), MESSAGE_DELIM, 0);
...
void ArduinoController::processMessage(unsigned char* data, int length)
{
    ...
    switch(msg.msg_type)
    {
        case ENCODER_MSG:
        {
            ...
            enc.header.stamp = this->get_clock()->now();
