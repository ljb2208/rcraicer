#include "../include/rcraicer/ubx_protocol.h"
#include <iostream>
#include <cstring>

UbxProtocol::UbxProtocol(rclcpp::Node::SharedPtr nodeHandle, bool msgDebug, std::string portPath, int baudRate, bool baseStation) : serialPort(NULL)
{
...
{
    serialPort = new SerialPort(nodeHandle, nodeHandle->get_name(), "gpsnode",  portPath, baudRate, '\0', 2);
    serialPort->registerByteDataCallback(std::bind(&UbxProtocol::serial_data_callback, this, std::placeholders::_1));    
...
{
    if (isGpsStatusMsgReady)
        serialPort->tick("GPSStatus Message pub");
...
{
    if (isGpsSurveyMsgReady)
        serialPort->tick("GPSSurvey Message pub");
...
bool UbxProtocol::navSatStatusMessageReady()
{
    ...
    if (isNavSatStatusMsgReady)
        serialPort->tick("NavSat Status Message pub");
...
bool UbxProtocol::navSatFixMessageReady()
{
    ...
    if (isNavSatFixMsgReady)
        serialPort->tick("NavSat Fix Message pub");
